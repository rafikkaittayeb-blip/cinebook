...
export default function HomePage() {
  ...
  return (
    <main className="min-h-screen bg-[#0a0a0a]">
      ...
      {showHero && !loadingMovies && featured && (
          ...
          <div className="grid lg:grid-cols-2 gap-4 mb-4">
            <Link href={`/movies/${featured.id}`} className="group relative rounded-2xl overflow-hidden aspect-[3/4] lg:aspect-auto lg:min-h-[580px] block">
              ...
              <div className="absolute top-4 left-4 flex gap-2">
                ...
                <span className="px-2.5 py-1 bg-black/60 backdrop-blur text-white text-xs rounded-lg border border-white/10">{featured.genre[0]}</span>
              </div>
              <div className="absolute bottom-0 left-0 right-0 p-6">
                <div className="flex flex-wrap gap-1.5 mb-3">
                  {featured.genre.slice(0, 3).map(g => <span key={g} className="genre-badge">{g}</span>)}
                </div>
                <h2 className="text-3xl sm:text-4xl font-serif font-bold text-white leading-tight mb-2">{featured.title}</h2>
                <p className="text-gray-300 text-sm leading-relaxed line-clamp-2 mb-4">{featured.description}</p>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3 text-sm text-gray-400">
                    <span className="flex items-center gap-1"><Star className="h-4 w-4 text-yellow-500 fill-yellow-500" /><span className="text-white font-bold">{featured.rating}</span></span>
                    <span className="flex items-center gap-1"><Clock className="h-4 w-4" />{featured.duration}m</span>
                  </div>
                  <span className="flex items-center gap-2 px-4 py-2 bg-yellow-500 text-black font-bold rounded-lg text-sm group-hover:bg-yellow-400 transition-colors">Book Now <ChevronRight className="h-4 w-4" /></span>
                </div>
              </div>
            </Link>
              ...
              {secondaryFeatured.map(movie => (
                <Link key={movie.id} href={`/movies/${movie.id}`} className="group relative rounded-xl overflow-hidden aspect-[2/3] block">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-3">
                    <p className="text-white font-serif font-bold text-sm leading-tight group-hover:text-yellow-400 transition-colors">{movie.title}</p>
        ...
        <div className="mx-auto max-w-7xl">
          ...
          {loadingMovies ? (
            ...
            <div className="flex flex-col items-center justify-center py-24 text-center">
              ...
              <button onClick={() => { setSearch(''); setActiveGenre('All') }} className="text-yellow-500 hover:text-yellow-400 text-sm underline">Clear filters</button>
            </div>
          ) : (
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
              {filtered.map(movie => (
                <Link key={movie.id} href={`/movies/${movie.id}`} className="group">
                  <div className="movie-card bg-[#111] border border-[#2a2a2a] rounded-xl overflow-hidden h-full flex flex-col">
                    ...
                    <div className="p-3 flex flex-col gap-1.5 flex-1">
                      <h3 className="font-serif font-bold text-white text-sm leading-tight group-hover:text-yellow-400 transition-colors line-clamp-1">{movie.title}</h3>
                      <div className="flex items-center gap-1.5 text-gray-600 text-xs">
                        <Clock className="h-3 w-3" /><span>{movie.duration}m</span><span>•</span><span>{movie.genre[0]}</span>
                      </div>
                      <p className="text-yellow-500 font-bold text-xs mt-auto pt-2 border-t border-[#1e1e1e]">From EGP {movie.price}</p>
                    </div>
                  </div>
                </Link>
              ))}
