...
import { SHOWTIMES, MOVIES, generateSeats, Seat } from '@/lib/mock-data'
      ...
      <div className="border-b border-[#1a1a1a] bg-[#0d0d0d]">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-4 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-gray-500 text-xs">{formatDate(showtime.date)} • {showtime.time} • {showtime.format}</p>
          </div>
        </div>
      </div>
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="flex items-center gap-6 text-xs text-gray-400">
              <div className="flex items-center gap-2">
                <div className="w-6 h-6 rounded seat-available border" />
                <span>Available</span>
              </div>
              <div className="flex items-center gap-2">
                    ...
                    {Array.from({ length: 12 }).map((_, idx) => {
                      ...
                      return (
                        <button
                          ...
                          disabled={isBooked}
                          title={isBooked ? 'Taken' : `${seatId} — EGP ${seat?.price}`}
            ...
            <div className="sticky top-24 bg-[#111] border border-[#2a2a2a] rounded-xl p-6 space-y-5">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between text-gray-400">
                  <span>Movie</span>
                  <span className="text-white text-right max-w-[120px] truncate">{movie.title}</span>
                </div>
                <div className="flex justify-between text-gray-400">
                  <span>Date</span>
                ...
                <div className="flex justify-between text-gray-400">
                  <span>Format</span>
                  <span className="text-yellow-400 font-medium">{showtime.format}</span>
                </div>
              </div>

              <div className="border-t border-[#1e1e1e] pt-4">
                <p className="text-sm text-gray-400 mb-3">
                  Selected Seats <span className="text-yellow-500 font-bold">({selectedSeats.length})</span>
                </p>
                {selectedSeats.length > 0 ? (
                  <div className="flex flex-wrap gap-1.5">
                    {selectedSeats.map(s => (
                      <span key={s} className="px-2.5 py-1 bg-yellow-500/10 border border-yellow-500/30 text-yellow-400 rounded-lg text-xs font-bold">
                        {s}
                      </span>
                    ))}
                  </div>
              ...
              <div className="border-t border-[#1e1e1e] pt-4 space-y-2 text-sm">
                <div className="flex justify-between text-gray-400">
                  <span>Subtotal</span>
                  <span className="text-white">EGP {totalPrice}</span>
                </div>
                <div className="flex justify-between text-gray-400">
                  <span>Booking fee</span>
                  <span className="text-white">EGP 10</span>
                </div>
