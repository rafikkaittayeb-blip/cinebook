...
import { ArrowLeft, CreditCard, Check, Lock, Ticket, Gift, Zap, TrendingUp } from 'lucide-react'
import { useAuth } from '@/lib/auth-context'

interface Showtime { id: string; date: string; time: string; format: string; price: number }
interface Movie { id: string; title: string }
interface Seat { id: string; row: string; number: number; price: number }
interface Voucher {
  id: string
  type: string
  description: string
  discountType: 'full' | 'fixed' | 'percentage'
  discountValue: number
}
interface BookingResult {
  bookingRef: string; movieTitle: string; showDate: string; showTime: string
  format: string; seats: string[]; totalPrice: number; qrCode: string
}

const VOUCHER_ICONS: Record<string, React.ElementType> = {
  free_ticket: Zap,
  concession_50: Gift,
  priority_month: TrendingUp,
}
...
function PaymentContent() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`/api/showtimes/${showTimeId}/seats`).then(r => r.json()),
      fetch('/api/loyalty/vouchers').then(r => r.json()),
    ]).then(([seatsData, vouchersData]) => {
      if (seatsData.showtime) setShowtime(seatsData.showtime)
      if (seatsData.movie) setMovie(seatsData.movie)
      const allSeats: Seat[] = seatsData.seats || []
      setSeatDetails(allSeats.filter(s => selectedSeatIds.includes(s.id)))
      const activeVouchers: Voucher[] = vouchersData.vouchers || []
      setVouchers(activeVouchers)
      // Auto-apply first available voucher
      if (activeVouchers.length > 0) setActiveVoucher(activeVouchers[0])
  ...
  if (activeVoucher) {
    if (activeVoucher.discountType === 'full') voucherDiscount = seatTotal + bookingFee
    else if (activeVoucher.discountType === 'fixed') voucherDiscount = Math.min(activeVoucher.discountValue, seatTotal + bookingFee)
    else if (activeVoucher.discountType === 'percentage') voucherDiscount = Math.round((seatTotal + bookingFee) * (activeVoucher.discountValue / 100) * 100) / 100
  }

  // Loyalty discount only applies if no voucher
  const loyaltyDiscount = (!activeVoucher && useLoyalty && user)
    ? Math.min(user.loyaltyPoints * 0.1, seatTotal * 0.2)
    : 0
  const total = Math.max(0, seatTotal + bookingFee - voucherDiscount - loyaltyDiscount)
  const isFreeBooking = activeVoucher?.discountType === 'full'
  ...
  const canSubmit = isFreeBooking || (!loading && formData.cardName && formData.cardNumber && formData.expiry && formData.cvv && !!user)

  const handleSubmit = async () => {
    if (!user) { router.push('/login'); return }
    if (!isFreeBooking && (!formData.cardName || !formData.cardNumber || !formData.expiry || !formData.cvv)) return
    setLoading(true)
    setError('')
    try {
      const res = await fetch('/api/bookings', {
        ...
        body: JSON.stringify({
          ...
          paymentMethod: isFreeBooking ? 'voucher' : paymentMethod,
          cardLast4: formData.cardNumber.replace(/\s/g, '').slice(-4) || null,
          useLoyalty: !activeVoucher && useLoyalty,
      ...
      <main className="min-h-screen bg-[#0a0a0a] flex items-center justify-center px-4">
        <div className="max-w-md w-full text-center space-y-6 fade-in">
          ...
          <div className="bg-[#111] border border-[#2a2a2a] rounded-xl p-6 text-left space-y-3 text-sm">
            ...
            <div className="flex justify-between"><span className="text-gray-400">Total Paid</span><span className="text-yellow-500 font-bold">{bookingResult.totalPrice === 0 ? 'FREE' : `EGP ${bookingResult.totalPrice.toFixed(2)}`}</span></div>
  ...
  const VoucherIcon = activeVoucher ? (VOUCHER_ICONS[activeVoucher.type] || Ticket) : Ticket
  ...
  return (
    <main className="min-h-screen bg-[#0a0a0a]">
      ...
      <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 py-10">
        ...
        {activeVoucher && (
          <div className="mb-6 p-4 rounded-xl bg-yellow-500/10 border-2 border-yellow-500/60 flex items-start gap-3 fade-in">
            <div className="p-2 rounded-lg bg-yellow-500/20 flex-shrink-0">
              <VoucherIcon className="h-5 w-5 text-yellow-400" />
            </div>
            <div className="flex-1">
              <p className="text-yellow-400 font-bold text-sm">🎟️ Voucher Applied: {activeVoucher.description}</p>
              {isFreeBooking
                ? <p className="text-gray-400 text-xs mt-1">This booking is completely free — no card required!</p>
                : <p className="text-gray-400 text-xs mt-1">Saving EGP {voucherDiscount.toFixed(2)} on this booking</p>}
            </div>
            <button onClick={() => setActiveVoucher(null)} className="text-gray-600 hover:text-gray-400 text-xs flex-shrink-0">Remove</button>
          </div>
        )}

        {/* Show other available vouchers if not applied */}
        {!activeVoucher && vouchers.length > 0 && (
          <div className="mb-6 p-4 rounded-xl bg-[#111] border border-yellow-500/20">
            <p className="text-yellow-400 text-sm font-medium mb-3">🎟️ You have {vouchers.length} voucher{vouchers.length > 1 ? 's' : ''} available:</p>
            <div className="flex flex-wrap gap-2">
              {vouchers.map(v => {
                const Icon = VOUCHER_ICONS[v.type] || Ticket
                return (
                  <button key={v.id} onClick={() => setActiveVoucher(v)}
                    className="flex items-center gap-2 px-3 py-2 rounded-lg bg-yellow-500/10 border border-yellow-500/30 text-yellow-400 text-xs font-medium hover:bg-yellow-500/20 transition-colors">
                    <Icon className="h-3.5 w-3.5" /> Apply: {v.description}
                  </button>
                )
              })}
            </div>
        ...
        <div className="grid lg:grid-cols-5 gap-8">
          <div className="lg:col-span-3 space-y-6">
            ...
            {/* Hide entire card form for free bookings */}
            {isFreeBooking ? (
              <div className="bg-[#111] border border-yellow-500/30 rounded-xl p-8 text-center space-y-4">
                <div className="h-16 w-16 rounded-full bg-yellow-500/10 border-2 border-yellow-500/40 flex items-center justify-center mx-auto">
                  <Zap className="h-8 w-8 text-yellow-500" />
                </div>
                <h2 className="text-xl font-serif font-bold text-white">Free Ticket Voucher Active</h2>
                <p className="text-gray-400 text-sm">Your voucher covers the full cost of this booking — no payment details needed.</p>
                <div className="bg-[#0d0d0d] rounded-lg p-3 inline-block">
                  <p className="text-yellow-500 text-2xl font-bold">EGP 0.00</p>
                  <p className="text-gray-600 text-xs mt-1">Total due</p>
                </div>
              </div>
            ) : (
              <>
                ...
                {user && user.loyaltyPoints > 0 && !activeVoucher && (
            ...
            <button onClick={handleSubmit} disabled={!canSubmit || loading}
              className={`w-full py-4 rounded-xl font-bold text-lg flex items-center justify-center gap-2 transition-all ${
                canSubmit && !loading ? 'bg-yellow-500 text-black hover:bg-yellow-400' : 'bg-[#1a1a1a] text-gray-600 cursor-not-allowed border border-[#2a2a2a]'
              }`}>
              {loading ? (
                <><div className="h-5 w-5 rounded-full border-2 border-black border-t-transparent animate-spin" />Processing...</>
              ) : isFreeBooking ? (
                <><Zap className="h-5 w-5" />Claim Free Ticket</>
              ) : (
                <><Lock className="h-5 w-5" />Pay EGP {total.toFixed(2)}</>
              )}
            </button>
            {!isFreeBooking && <p className="text-center text-gray-600 text-xs">Your payment is encrypted and secure</p>}
          </div>
          ...
          <div className="lg:col-span-2">
            <div className="sticky top-24 bg-[#111] border border-[#2a2a2a] rounded-xl p-6 space-y-5">
              ...
              <div className="border-t border-[#1e1e1e] pt-4 space-y-2 text-sm">
                <div className="flex justify-between text-gray-400"><span>{seatDetails.length} seat(s)</span><span className="text-white">EGP {seatTotal}</span></div>
                <div className="flex justify-between text-gray-400"><span>Booking fee</span><span className="text-white">EGP {bookingFee}</span></div>
                {activeVoucher && voucherDiscount > 0 && (
                  <div className="flex justify-between text-yellow-400">
                    <span className="flex items-center gap-1"><VoucherIcon className="h-3.5 w-3.5" /> {activeVoucher.description}</span>
                    <span>- EGP {voucherDiscount.toFixed(2)}</span>
                  </div>
                )}
                {!activeVoucher && useLoyalty && loyaltyDiscount > 0 && (
                ...
                <div className="flex justify-between items-center pt-3 border-t border-[#1e1e1e]">
                  ...
                  <span className="text-2xl font-bold text-yellow-500">{total === 0 ? 'FREE' : `EGP ${total.toFixed(2)}`}</span>
                </div>
              </div>
            </div>
          </div>
