...
export default function LoyaltyPage() {
  ...
  const tier = data?.tier ?? user?.tier ?? 'Bronze'
    ...
    return (
      <main className="min-h-screen bg-[#0a0a0a] flex items-center justify-center px-4">
        <div className="text-center space-y-4">
          <Star className="h-16 w-16 text-yellow-500/40 mx-auto" />
          <h1 className="text-2xl font-serif font-bold text-white">Sign in to view loyalty rewards</h1>
          <p className="text-gray-500 text-sm">Create an account and start earning points with every booking.</p>
          <div className="flex gap-3 justify-center">
            <Link href="/login" className="px-6 py-2.5 rounded-xl border border-yellow-500/30 text-yellow-400 hover:bg-yellow-500/10 transition-colors text-sm font-medium">Sign In</Link>
            <Link href="/register" className="px-6 py-2.5 rounded-xl bg-yellow-500 text-black font-bold hover:bg-yellow-400 transition-colors text-sm">Register</Link>
          </div>
        </div>
      </main>
    )
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            ...
            <div className="bg-[#111] border border-[#2a2a2a] rounded-xl p-5">
              ...
              <p className="text-3xl font-bold text-white">{data?.thisMonth ?? 0}</p>
            ...
            <div className={`bg-[#111] border rounded-xl p-5 ${tierInfo.bg}`}>
              ...
              <p className={`text-2xl font-bold ${tierInfo.color}`}>{tier} Member</p>
              ...
              <div className="space-y-4">
                {[
                  ...
                  <div key={item.step} className="flex gap-4">
                    <div className="h-9 w-9 rounded-full bg-yellow-500 text-black font-bold text-sm flex items-center justify-center flex-shrink-0">{item.step}</div>
                    <div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid sm:grid-cols-3 gap-3">
                {[
                  ...
                  <div key={t.name} className={`rounded-xl p-4 border-2 transition-all ${t.current ? 'border-yellow-500 bg-yellow-500/5' : 'border-[#2a2a2a] bg-[#111]'}`}>
                    {t.current && <span className="inline-block px-2 py-0.5 bg-yellow-500 text-black text-xs font-bold rounded-full mb-2">Current</span>}
                    <h3 className="font-serif font-bold text-white mb-0.5">{t.name}</h3>
                    <p className="text-gray-600 text-xs mb-3">{t.min === '0' ? 'Starting tier' : `${t.min}+ points`}</p>
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-24 bg-[#111] border border-[#2a2a2a] rounded-xl p-5 space-y-4">
              ...
              {data?.transactions.length ? (
                ...
                <p className="text-gray-600 text-sm">No transactions yet. Book a movie to earn points!</p>
              )}
              <Link href="/my-bookings" className="block w-full text-center py-2 rounded-lg border border-yellow-500/30 text-yellow-400 hover:bg-yellow-500/10 transition-colors text-sm font-medium">View All Bookings</Link>
            </div>
          </div>
        </div>
      </div>
